...
// using namespace viz_arm_neon;

bool viz_arm_neon::resize_nv21_or_nv12_buffer(char* yuv, char* yuv_resized, const int image_width, const int image_height, const int target_width, const int target_height, const unified_detector::UnifiedDetector::UNIFIED_DETECTOR_EXECUTE_PARAM::ImageFormat image_format){
...
static inline void viz_arm_neon::ics_yuv2rgb_bt709_neon_8I(uint8x8_t y,  uint8x8_t cb,   uint8x8_t cr,   uint8x8_t *r, uint8x8_t *g, uint8x8_t *b){
    /*
...
static inline void viz_arm_neon::ics_yuv2rgb_bt709_neon(unsigned char y, unsigned char cb, unsigned char cr,
...
//https://software-dl.ti.com/jacinto7/esd/processor-sdk-rtos-jacinto7/06_02_00_21/exports/docs/vxlib_c66x_1_1_4_0/docs/doxygen/html/vxlib_html/group___v_x_l_i_b__color_convert___y_u_v_xto_r_g_b__i8u__o8u.html
    ...
    int i_g = (int)f_g;
    int i_b = (int)f_b;
    *r = viz_arm_neon::ics_usat8_neon(i_r);
    *g = viz_arm_neon::ics_usat8_neon(i_g);
    *b = viz_arm_neon::ics_usat8_neon(i_b);

    return;
}

static inline unsigned char viz_arm_neon::ics_usat8_neon(int a){
    if (a > 255)
        a = 255;
    if (a < 0)
        a = 0;
...
int viz_arm_neon::ics_fconvert_nv21_or_nv12_torgb_neon(unsigned char* in, unsigned char* out, int width, int height, const unified_detector::UnifiedDetector::UNIFIED_DETECTOR_EXECUTE_PARAM::ImageFormat image_format){
    int in_stride = width;
    int width1   = (width/(16));
    width1 = (width1)*16;
    for (int y = 0; y < height; y+=2){
        ...
        for (x = 0; x < width1; x+=16){
            ...
            if(image_format==unified_detector::UnifiedDetector::UNIFIED_DETECTOR_EXECUTE_PARAM::ImageFormat::NV21){
                cb = vzip_u8(cbcr.val[1], cbcr.val[1]);
                cr = vzip_u8(cbcr.val[0], cbcr.val[0]);
            }else{
                cb = vzip_u8(cbcr.val[0], cbcr.val[0]);
                cr = vzip_u8(cbcr.val[1], cbcr.val[1]);
            }

            viz_arm_neon::ics_yuv2rgb_bt709_neon_8I(luma00, cb.val[0], cr.val[0], &bgr00.val[2], &bgr00.val[1], &bgr00.val[0]);
            viz_arm_neon::ics_yuv2rgb_bt709_neon_8I(luma01, cb.val[1], cr.val[1], &bgr01.val[2], &bgr01.val[1], &bgr01.val[0]);
            viz_arm_neon::ics_yuv2rgb_bt709_neon_8I(luma10, cb.val[0], cr.val[0], &bgr10.val[2], &bgr10.val[1], &bgr10.val[0]);
            viz_arm_neon::ics_yuv2rgb_bt709_neon_8I(luma11, cb.val[1], cr.val[1], &bgr11.val[2], &bgr11.val[1], &bgr11.val[0]);
        ...
        for (; x < width; x+=2){
            ...
            if(image_format==unified_detector::UnifiedDetector::UNIFIED_DETECTOR_EXECUTE_PARAM::ImageFormat::NV21){
                viz_arm_neon::ics_yuv2rgb_bt709_neon(luma1[0], cbcr[1], cbcr[0], &bgr1[2], &bgr1[1], &bgr1[0]);
                viz_arm_neon::ics_yuv2rgb_bt709_neon(luma1[1], cbcr[1], cbcr[0], &bgr1[5], &bgr1[4], &bgr1[3]);
                viz_arm_neon::ics_yuv2rgb_bt709_neon(luma2[0], cbcr[1], cbcr[0], &bgr2[2], &bgr2[1], &bgr2[0]);
                viz_arm_neon::ics_yuv2rgb_bt709_neon(luma2[1], cbcr[1], cbcr[0], &bgr2[5], &bgr2[4], &bgr2[3]);
            }else{
                viz_arm_neon::ics_yuv2rgb_bt709_neon(luma1[0], cbcr[0], cbcr[1], &bgr1[2], &bgr1[1], &bgr1[0]);
                viz_arm_neon::ics_yuv2rgb_bt709_neon(luma1[1], cbcr[0], cbcr[1], &bgr1[5], &bgr1[4], &bgr1[3]);
                viz_arm_neon::ics_yuv2rgb_bt709_neon(luma2[0], cbcr[0], cbcr[1], &bgr2[2], &bgr2[1], &bgr2[0]);
                viz_arm_neon::ics_yuv2rgb_bt709_neon(luma2[1], cbcr[0], cbcr[1], &bgr2[5], &bgr2[4], &bgr2[3]);
            }
        }
    }
...
// }
cv::Mat viz_arm_neon::nv21_or_nv12_resize_convert_rotate(char* yuv, const int image_width, int const image_height, int const target_width, int const target_height, int rotation_angle, 
    ...
    viz_arm_neon::resize_nv21_or_nv12_buffer(yuv, yuv_resized, image_width, image_height, target_width, target_height, image_format);

    // convert
    cv::Mat image(target_height, target_width, CV_8UC3);
    unsigned char *bgr = (unsigned char*) image.data;
    viz_arm_neon::ics_fconvert_nv21_or_nv12_torgb_neon(reinterpret_cast<unsigned char*>(yuv_resized), bgr, target_width, target_height, image_format);
    delete[] yuv_resized;
    // unsigned char *bgr = new unsigned char[image_height * image_width * 3];
    // viz_arm_neon::ics_fconvert_nv21torgb_neon(reinterpret_cast<unsigned char*>(yuv), bgr, image_width, image_height);
...
cv::Mat viz_arm_neon::nv21_or_nv12_convert_resize_rotate(char* yuv, const int image_width, int const image_height, int const target_width, int const target_height, int rotation_angle, 
    const unified_detector::UnifiedDetector::UNIFIED_DETECTOR_EXECUTE_PARAM::ImageFormat image_format) {
    // convert
    //LOGV("VZ Debug: nv21_convert_resize_rotate image_width = [%d], image_height=[%d], target_width=[%d], target_height=[%d]", image_width, image_height, target_width, target_height);
    cv::Mat image(image_height, image_width, CV_8UC3);
    unsigned char *bgr = (unsigned char*) image.data;
    viz_arm_neon::ics_fconvert_nv21_or_nv12_torgb_neon(reinterpret_cast<unsigned char*>(yuv), bgr, image_width, image_height, image_format);
