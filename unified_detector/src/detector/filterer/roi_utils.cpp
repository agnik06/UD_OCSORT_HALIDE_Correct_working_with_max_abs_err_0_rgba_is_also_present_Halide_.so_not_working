...
std::vector<unified_detector::BoundingBox> ROIUtils::remove_smaller_boxes(std::vector<unified_detector::BoundingBox> boxes, int img_w, int img_h, float ratio_threshold, int num_boxes){
    ...
    for (int i = 0; i < boxes.size(); i++) {
        ...
        if (ratio*100 > ratio_threshold*100) {
...
std::vector<unified_detector::BoundingBox> ROIUtils::remove_smaller_boxes_for_Specific_categories(std::vector<unified_detector::BoundingBox> boxes, int img_w, int img_h, int num_boxes){
    std::vector<unified_detector::BoundingBox> selected_boxes;
    ...
    for (int i = 0; i < boxes.size(); i++) {
        ...
        int minAreaBoxThreshold = 0.02*100;
        int minAreaBoxThresholdPetface = 0.02*100;
        int minAreaBoxThresholdPet = 0.10*100;
        ...
        /*if(!((boxes[i].tag == "flower" || boxes[i].tag == "pets") && (areaThreshold < minAreaBoxThreshold))){
            selected_boxes.push_back(boxes[i]);
        }*/
        if(boxes[i].tag == "petface" && (areaThreshold > minAreaBoxThresholdPetface)){
            selected_boxes.push_back(boxes[i]);
            continue;
        }
        if(boxes[i].tag == "pets" && (areaThreshold > minAreaBoxThresholdPet)){
            
            selected_boxes.push_back(boxes[i]);
            continue;
        }else{
            //LOGV("Removing the pets because of min area threshold with score [%f], areaThreshold : [%d]", boxes[i].score, areaThreshold);
        }
        if(!((boxes[i].tag == "flower") && (areaThreshold < minAreaBoxThreshold))){
            selected_boxes.push_back(boxes[i]);
        }
    }
    return selected_boxes;
...
float ROIUtils::get_overlap_between_boxes(unified_detector::Rectangle box1, unified_detector::Rectangle box2) {
...
float ROIUtils::get_iou_between_boxes(unified_detector::Rectangle box1, unified_detector::Rectangle box2){
    ...
    unified_detector::Rectangle intersect_box (inter_left, inter_top, inter_right, inter_bottom);
...
std::vector<unified_detector::BoundingBox> ROIUtils::remove_overlapping_boxes(std::vector<unified_detector::BoundingBox> secondary_boxes,
                                                                 const std::vector<unified_detector::BoundingBox> primary_boxes,
                                                                 float iou_threshold){
    secondary_boxes.erase(
        std::remove_if(secondary_boxes.begin(), secondary_boxes.end(), 
        [primary_boxes, iou_threshold](const unified_detector::BoundingBox& secondary_box){
            for(unified_detector::BoundingBox primary_box: primary_boxes){
...
int ROIUtils::check_overlap_box(unified_detector::Rectangle r1,unified_detector::Rectangle r2,float area_threshold){
...
std::vector<unified_detector::BoundingBox> ROIUtils::remove_other_boxes_in_category_box(std::vector<unified_detector::BoundingBox> boxes,std::vector<std::string> &categories,std::vector<float> &thresholds){
...
std::vector<unified_detector::BoundingBox> ROIUtils::remove_bigger_boxes(std::vector<unified_detector::BoundingBox> all_boxes, int img_w, int img_h){
    std::vector<unified_detector::BoundingBox> selected_boxes;
    std::vector<unified_detector::BoundingBox> boxes;
    unified_detector::BoundingBox save_box;
    ...
    for (int i = 0; i < all_boxes.size(); i++) {
        if (all_boxes[i].tag == "person") {
            ...
            float area_ratio = all_boxes[i].location.area() / (img_w * img_h * 1.0);
            if (area_ratio < 0.8) {
...
cv::Mat ROIUtils::getGaussianMatrix_HQ(cv::Mat image_src, int width, int height, double var, int mode){
    ...
    cv::cvtColor(image_src, greyScale, CV_BGR2GRAY);  // CV_YUV2BGR_NV21 = 93,
...
std::vector<unified_detector::BoundingBox> ROIUtils::get_top_boxes(std::vector<unified_detector::BoundingBox>& boxes, int img_w, int img_h, int num_boxes){
...
double ROIUtils::getVariance_HQ(cv::Mat image_src, int width, int height, double var, int mode) {
    ...
    cv::resize(image_src, resizedImg, cv::Size(width, height), 0, 0, CV_INTER_LINEAR);

    cv::Mat greyScale(height, width, CV_8UC1);
    cv::cvtColor(resizedImg, greyScale, CV_BGR2GRAY);  // CV_YUV2BGR_NV21 = 93,
...
std::vector<unified_detector::BoundingBox> ROIUtils::get_top_boxes_HQ( std::vector<unified_detector::BoundingBox>& boxes, int img_w, int img_h, cv::Mat image_src, int num_boxes){
    std::vector<unified_detector::BoundingBox> candidate_boxes;
    std::vector<unified_detector::BoundingBox> top_boxes;

    std::vector<unified_detector::BoundingBox> inside_boxes;
    std::vector<unified_detector::BoundingBox> outside_boxes;
    unified_detector::Rectangle loc;
    int taken = 0;
    if (boxes.size() <= 1) {
        return boxes;
        ...
        for (unified_detector::BoundingBox box : inside_boxes) {
            loc = box.location;
            candidate_boxes.push_back(box);
        }
        for (unified_detector::BoundingBox box : outside_boxes) {
            loc = box.location;
            candidate_boxes.push_back(box);
        }
        for (unified_detector::BoundingBox box : candidate_boxes) {
            taken++;
            loc = box.location;
...
/*bool ROIUtils::isCentered(unified_detector::Rectangle roi_location, int width, int height, float margin){
    float margin_sqrt = sqrt(margin);
    unified_detector::Rectangle center_rect = { (int)(width * (1 - margin_sqrt) * 0.5),
...
std::vector<unified_detector::BoundingBox> ROIUtils::sort_boxes(std::vector<unified_detector::BoundingBox> boxes, std::vector<float> scores){
    ...
    std::vector<unified_detector::BoundingBox> sorted_boxes;
...
std::vector<unified_detector::BoundingBox> ROIUtils::soft_non_max_suppression_fast(std::vector<unified_detector::BoundingBox>& boxes,
                                                  int imgW, int imgH) {
  std::vector<unified_detector::BoundingBox> picked;
  if (boxes.size() == 0) {
    return std::vector<unified_detector::BoundingBox>();
  }
