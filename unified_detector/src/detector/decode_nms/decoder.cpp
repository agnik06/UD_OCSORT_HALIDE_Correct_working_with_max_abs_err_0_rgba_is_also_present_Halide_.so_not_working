...
static void
generate_grid_center_priors(const int input_width, const int input_height,
...
Decoder::Decoder(int _target_width,int _target_height, model::Version version) {
  ...
  if(version == model::Version::V2){
    this->num_class = v2_labels.size();
    this->labels = v2_labels;
    LOGV("VZ Debug:  Decoder::Decoder version 2 num_class : [%d]", this->num_class);
  }else {
    this->num_class = v1_labels.size();
    this->labels = v1_labels;
    LOGV("VZ Debug:  Decoder::Decoder version 1 num_class : [%d]", this->num_class);
  }
  LOGV("VZ Debug:  Decoder::Decoder this->input_size[0] : [%d], this->input_size[1] :[%d], number of classes : [%d]", this->target_width, this->target_height,this->num_class);
...
std::vector<std::vector<float> > Decoder::decode(Tensor2D<float> &feats,
  ...
  //std::vector<DecodeBoxInfo> dets;
  std::vector<std::vector<float> > final_rois; // 4 coordinates 1 class and other confidence score.
  final_rois.clear();
  for (int i = 0; i < (int)results.size(); i++) {
    //LOGV("VZ Debug:  Decoder::decode pre-nms resultssize=[%d]", results[i].size());
    this->nms(results[i], nms_threshold);
    //LOGV("VZ Debug:  Decoder::decode post-nms resultssize=[%d]", results[i].size());
...
void Decoder::set_target_dimen(int _target_width,int _target_height){
  this->target_width = _target_width;
  this->target_height =_target_height;
  LOGV("Decoder : set_target_dimen : target_width : [%d], target_height : [%d], number of classes : [%d]", this->target_width, this->target_height,this->num_class);
}
...
DecodeBoxInfo Decoder::disPred2Bbox(const float *dfl_det, int label,
  ...
  // std::endl;
  return DecodeBoxInfo{xmin, ymin, xmax, ymax, score, label};
}

void Decoder::nms(std::vector<DecodeBoxInfo> &input_boxes, float NMS_THRESH) {
  std::sort(input_boxes.begin(), input_boxes.end(),
            [](DecodeBoxInfo a, DecodeBoxInfo b) { return a.score > b.score; });
