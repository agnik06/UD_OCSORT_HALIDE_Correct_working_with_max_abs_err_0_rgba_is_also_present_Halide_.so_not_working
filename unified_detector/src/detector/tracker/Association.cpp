...
namespace ocsort {
    ...
    std::tuple<std::vector<Eigen::Matrix<int, 1, 2>>, std::vector<int>, std::vector<int>> associate(Eigen::MatrixXf detections, Eigen::MatrixXf trackers, float iou_threshold, Eigen::MatrixXf velocities, Eigen::MatrixXf previous_obs_, float vdc_weight) {
        ...
        Eigen::MatrixXf angle_diff_cost = ((((valid_mask_.cast<float>()).array() * diff_angle.array()).array() * vdc_weight)
            ...
            .array() *
            scores.array();
        // Accumulate matches into a vector first, then copy to Eigen matrix once.
        // Avoids per-iteration conservativeResize (which is O(N) per call -> O(N^2) overall).
        ...
        if (std::min(iou_matrix.cols(), iou_matrix.rows()) > 0) {
            ...
            if ((fabs(sum1 - 1) < 1e-12) && (fabs(sum0 - 1) < 1e-12)) {
                match_pairs.reserve(static_cast<size_t>(std::min(a.rows(), a.cols())));
                ...
                match_pairs.reserve(rowsol.size());
                for (size_t i = 0; i < rowsol.size(); i++) {
                    if (rowsol.at(i) >= 0) {
                        match_pairs.emplace_back(colsol.at(rowsol.at(i)), static_cast<int>(rowsol.at(i)));
                    }
                }
            }
        }
        // Single allocation copy from the accumulator into the Eigen matrix
        Eigen::Matrix<float, Eigen::Dynamic, 2> matched_indices(static_cast<int>(match_pairs.size()), 2);
        for (size_t k = 0; k < match_pairs.size(); k++) {
            matched_indices(static_cast<int>(k), 0) = static_cast<float>(match_pairs[k].first);
            matched_indices(static_cast<int>(k), 1) = static_cast<float>(match_pairs[k].second);
        }
        // O(N+M) using a flagged bool array instead of O(N*M) Eigen reductions per index.
        const int n_dets = static_cast<int>(detections.rows());
        const int n_trks = static_cast<int>(trackers.rows());
        std::vector<char> det_matched(n_dets, 0);
        std::vector<char> trk_matched(n_trks, 0);
        for (int k = 0; k < matched_indices.rows(); k++) {
            const int di = static_cast<int>(matched_indices(k, 0));
            const int ti = static_cast<int>(matched_indices(k, 1));
            if (di >= 0 && di < n_dets) det_matched[di] = 1;
            if (ti >= 0 && ti < n_trks) trk_matched[ti] = 1;
        }
        std::vector<int> unmatched_detections;
        unmatched_detections.reserve(n_dets);
        for (int i = 0; i < n_dets; i++) {
            if (!det_matched[i]) unmatched_detections.push_back(i);
        }
        std::vector<int> unmatched_trackers;
        unmatched_trackers.reserve(n_trks);
        for (int i = 0; i < n_trks; i++) {
            if (!trk_matched[i]) unmatched_trackers.push_back(i);
        }
