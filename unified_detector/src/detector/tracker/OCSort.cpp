...
//         // Pre-size to upper bound (= total rows) and fill, then shrink once.
//         // Avoids O(N^2) memory traffic from per-iteration conservativeResize.
//         Eigen::Matrix<float, Eigen::Dynamic, 6> dets_first(output_results.rows(), 6);
//         Eigen::Matrix<float, Eigen::Dynamic, 6> dets_second(output_results.rows(), 6);
//         int dets_first_count = 0;
//         int dets_second_count = 0;
//         for (int i = 0; i < output_results.rows(); i++) {
//             if (true == inds_second(i)) {
//                 dets_second.row(dets_second_count++) = output_results.row(i);
//             }
//             if (true == remain_inds(i)) {
//                 dets_first.row(dets_first_count++) = output_results.row(i);
//             }
//         }
//         dets_first.conservativeResize(dets_first_count, Eigen::NoChange);
//         dets_second.conservativeResize(dets_second_count, Eigen::NoChange);
//         // Reuse cached matrices instead of allocating fresh per frame.
//         // resize() preserves capacity and is a no-op if size unchanged.
//         const int n_trk = static_cast<int>(trackers.size());
//         cached_trks_.resize(n_trk, 5);
//         cached_velocities_.resize(n_trk, 2);
//         cached_last_boxes_.resize(n_trk, 5);
//         cached_k_observations_.resize(n_trk, 5);
//         // Use references to the cached matrices so the rest of the function
//         // reads identically to the original code path.
//         Eigen::MatrixXf& trks = cached_trks_;
//         Eigen::MatrixXf& velocities = cached_velocities_;
//         Eigen::MatrixXf& last_boxes = cached_last_boxes_;
//         Eigen::MatrixXf& k_observations = cached_k_observations_;
...
namespace ocsort {
    ...
    std::vector<Eigen::RowVectorXf> OCSort::update(Eigen::MatrixXf dets) {
        ...
        for (int i : unmatched_dets) {
            Eigen::VectorXf tmp_bbox = dets_first.block(i, 0, 1, 5).transpose();
            int cls_ = int(dets(i, 5));
            trackers.emplace_back(tmp_bbox, cls_, delta_t);
        }
        const int tracker_count = static_cast<int>(trackers.size());
