...
void* get_dlinterface_lib_handle() {
  ...
  dlinterface_lib_handle = dlopen("libDLInterface.camera.samsung.so", RTLD_LAZY);
  //dlinterface_lib_handle = dlopen("libDLInterface.so", RTLD_LAZY);
  if (dlinterface_lib_handle != NULL) {
...
#ifdef DLINTERFACELIBLOCATION_SYSTEM_AIDL
    if(dlinterface_lib_handle == NULL){
        dlinterface_lib_handle = dlopen(DLINTERFACELIBLOCATION_SYSTEM_AIDL, RTLD_LAZY);
    if (dlinterface_lib_handle != NULL) {
      LOGV("VZ Debug: Dynamic loading of libDLInterface successfull from %s",DLINTERFACELIBLOCATION_SYSTEM_AIDL);
    }
        else{
      LOGV("VZ Debug: Dynamic loading of libDLInterface from %s failed",DLINTERFACELIBLOCATION_SYSTEM_AIDL);
        }
    }
#endif


#ifdef DLINTERFACELIBLOCATION_SYSTEM_HIDL
    if(dlinterface_lib_handle == NULL){
        dlinterface_lib_handle = dlopen(DLINTERFACELIBLOCATION_SYSTEM_HIDL, RTLD_LAZY);
    if (dlinterface_lib_handle != NULL) {
      LOGV("VZ Debug: Dynamic loading of libDLInterface successfull from %s",DLINTERFACELIBLOCATION_SYSTEM_HIDL);
    }
        else{
      LOGV("VZ Debug: Dynamic loading of libDLInterface from %s failed",DLINTERFACELIBLOCATION_SYSTEM_HIDL);
        }
    }
#endif
	...
	if(dlinterface_lib_handle == NULL){
		dlinterface_lib_handle = dlopen(DLINTERFACELIBLOCATION_VENDOR, RTLD_LAZY);
		if (dlinterface_lib_handle != NULL) {
