...
function create_release_package(){
	...
	cp $APP_DIR/image_segmenter/build/outputs/aar/image_segmenter-debug.aar $RELEASE_DIR/App/$AAR_NAME-debug.aar
	cp $APP_DIR/image_segmenter/build/outputs/aar/image_segmenter-release.aar $RELEASE_DIR/App/$AAR_NAME-release.aar
...
function release_libs_hwasan(){
	source variables.sh
	libs_type=$1
	RELEASE_DIR_HWASAN=./releases/unified-detector
...
cp $ENGINE_DIR/../../unified_detector/include/unified_detector/* $RELEASE_DIR/Libs/
