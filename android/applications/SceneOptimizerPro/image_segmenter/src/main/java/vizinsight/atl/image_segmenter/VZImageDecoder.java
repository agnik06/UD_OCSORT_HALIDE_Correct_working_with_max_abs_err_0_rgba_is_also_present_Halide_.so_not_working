...
public class VZImageDecoder
{
    ...
    public static byte [] decodeImage(Bitmap bitmap, int image_format)
    {
        Log.i("BIPLAB_DEBUG", "Decoding Image ...");
        //image_format = IMAGE_FORMAT_YUV420;
        Log.i("BIPLAB_DEBUG", "Decoding Image, Image format being set to IMAGE_FORMAT_YUV420 ...");
    ...
    static byte [] getRGBA(int inputWidth, int inputHeight, Bitmap bitmap)
    {
    ...
    static byte [] getNV21(int inputWidth, int inputHeight, Bitmap bitmap)
    {
        ...
        Log.i("BIPLAB_DEBUG", "byte[N*M + ((N+(N%2))*(M+(M%2))/2)]");
        //byte [] yuv = new byte[inputWidth*inputHeight*3/2];
        int M = inputWidth;
        int N = inputHeight;
        byte [] yuv = new byte[N*M + ((N+(N%2))*(M+(M%2))/2)];
